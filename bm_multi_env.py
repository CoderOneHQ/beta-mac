...
'''
    ...
    def convert_video(self):
        '''
        Armin here it converts the recorded photos to a video
        '''
        folder = "./temp_photo"
        os.makedirs(folder,exist_ok=True)
        ...
        for count in range(len(glob.glob(folder + '/*.png'))):
            filename = folder+'/match{0:02}.png'.format(count+1)
        ...
        project_name = "/"+"p1" + self.players[0].name.replace(" ", "") + "_vs_" + "p2" + self.players[1].name.replace(" ", "") + datetime.datetime.now().strftime("%Y-%m-%d-_%H_%M_%S") + ".mp4"
        print('generating mp4 file of the match')
        out = cv2.VideoWriter(folder+project_name,cv2.VideoWriter_fourcc(*'mp4v'), 1, size)
